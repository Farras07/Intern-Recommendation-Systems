...
import {formVacancySchema, formBatchSchema, formRoleSchema} from "@/constant/schemas.items"
          ...
          <DialogContent className="sm:max-w-[425px]">
            ...
            {target === "Vacancy" && (
              <div className="grid gap-4">
                <Form {...formVacancy}>
                  <form onSubmit={formVacancy.handleSubmit(onSubmitVacancy)} className="space-y-8">
                    <FormField
                        ...
                        <FormItem className="flex flex-col">
                          ...
                          <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                <Button variant="outline">{selectedBatch || "Choose Batch"}</Button>
                            </DropdownMenuTrigger>
                            <DropdownMenuContent >
                                ...
                                <FormControl>
                                  <DropdownMenuRadioGroup>
                                    {batchData.map((batch) => (
                                      <DropdownMenuItem
                                        key={batch.batchId}
                                        onClick={() => formVacancy.setValue("batch", batch.batchName, { shouldValidate: true })}
                                      >
                                        {batch.batchName}
                                      </DropdownMenuItem>
                                    ))}
                                ...
                                <FormControl>
                                  <DropdownMenuRadioGroup>
                                    {roleData.map((role) => (
                                      <DropdownMenuItem
                                        key={role.id}
                                        onClick={() => formVacancy.setValue("role", role.title, { shouldValidate: true })}
                                      >
                                        {role.title}
                                      </DropdownMenuItem>
                                    ))}
                    ...
                    {fields.map((field, index) => (
                      <div key={field.id} className="grid grid-cols-4 gap-2">
                        ...
                        {/* Remove button */}
                        <Button type="button" onClick={() => remove(index)} className="place-self-end">Remove</Button>
                      </div>
                    ))}
            ...
            {target === "Role" && (
              <div className="grid gap-4">
                <Form {...formRole}>
                  <form onSubmit={formRole.handleSubmit(onSubmitRole)} className="space-y-8">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input
                              placeholder={action === "Edit"? data.title: "Type Role Title"}
                              {...field}
                            />
                          </FormControl>
                          <FormMessage />
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Textarea
                              placeholder={action==="Edit"? data.description : "Type Role Description"}
            ...
            {target === "Batch" && (
              <Form {...formBatch}>
                <form onSubmit={formBatch.handleSubmit(onSubmitBatch)} className="space-y-8">
                  <FormField
...
const addRoleSubmit = async ({ title, description }: { title: string, description: string}) => {
    try {
