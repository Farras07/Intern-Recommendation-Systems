            ...
            <div className="grid grid-cols-4 grid-rows-[fit-content(100%)_1fr] gap-3 h-full ">
                ...
                <CollapsibleContainer
                    ...
                    <div className="flex flex-col items-center gap-6">
                        <Typography variant="h6" weight="semibold">Team</Typography>
                        <div className="w-full flex flex-col gap-2">
                            {isPending && (
                                <Loading/>
                            )}
                            {Array.isArray(data?.user) && data.user.map((user: any) => (
                                <TeamCard
                                    key={user.email}
                                    name={user.name}
                                    image={user.image}
                    ...
                    <div className="mt-auto w-full text-center">
                        <Button variant="outline-black" className="cursor-pointer" onClick={
                            () => dialogToggle({target: "Team", action: "Add"})
                        }>
                            <UserPlus fill="black"/>
                            <Typography variant="btn" weight="semibold">Invite</Typography>
                        </Button>
                    </div>
                </CollapsibleContainer>
            </div>
