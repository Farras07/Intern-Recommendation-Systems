...
import { BatchPayloadUploadType, BatchTableTypes } from '@/types/BatchTypes'
...
export default function Vacancy() {
       ...
       sourceVacancy.addEventListener("vacancy_update", async (event) => {
        ...
        const fixData = await Promise.all(
            vacancyData.map(async (data:any, index: number) => {
                let status = "Pending";
                const { batch } = await _Fetch(`/intern/batch?name=${data.batch}`, "GET");
                ...
                if (currentDate < batchEndDateTime && currentDate > batchStartDateTime) status = "Hiring";
    ...
    const {email, name, image, role}: SessionUserData = session.user as SessionUserData
        ...
        <Layout>
            <section className="grid grid-cols-2 gap-3 h-full grid-rows-[fit-content(100%)_1fr]">
                ...
                <CollapsibleContainer
                    selfIndex={2}
                    onClick={() => handleToggleCollapsibleContainer({index: 2, setState:setActiveIndex})}
                    activeIndex={activeIndex}
                    rowSpan={6}
                    className='row-span-6'
                >
                    <div className='flex gap-4 items-center'>
                        <Typography variant="h6" weight="semibold">Vacancy</Typography>
                        <Plus color='white' strokeWidth={3} size={20} className='cursor-pointer rounded-full bg-black' onClick= 
                        {
                            () => dialogToggle({ target: "Vacancy", action: "Add" })
                        }/>
                    </div>
                    <div className='mt-5 flex flex-col gap-5'>
                        <DataTable columns={columns({dialogToggle, setFormState})} data={vacancyData}/>
                    </div>
                    
                </CollapsibleContainer>
                <CollapsibleContainer
                    selfIndex={1}
                    onClick={() => handleToggleCollapsibleContainer({index: 1, setState:setActiveIndex})}
                    activeIndex={activeIndex}
                    rowSpan={6}
                    className='row-span-5'
                >
                    <div className='flex gap-4 items-center'>
                        <Typography variant="h6" weight="semibold">Batch</Typography>
                        <Plus color='white' strokeWidth={3} size={20} className='cursor-pointer rounded-full bg-black' onClick= 
                        {
                            () => dialogToggle({ target: "Batch", action: "Add" })
                        }/>
                    </div>
                    <div className='mt-5 flex flex-col gap-5'>
                        <DataTable columns={columnsBatch({dialogToggle, setFormState})} data={batchData}/>
                    </div>
                ...
                <CollapsibleContainer
                    collapsible={false}
                    selfIndex={3}
                    onClick={() => handleToggleCollapsibleContainer({index: 3, setState:setActiveIndex})}
                    activeIndex={activeIndex}
                >
                    <Typography variant="h6" weight="semibold">Role</Typography>
                    <section className='mt-4 flex flex-wrap items-center gap-3'>
                        ...
                        >
                            ...
                            <Typography variant="c2" weight="semibold" color='white'>Add Role</Typography>
                        </Button>
                        {
                            rolesData.length > 0 ? (
                                ...
                                if (index < 6) {
                                    ...
                                    return (
                                        <Button key={role.id} className='p-0 pr-2 border-0 group cursor-pointer'>
                                            <div className='border border-black bg-background shadow-md rounded-[18px] hover:bg-accent hover:text-accent-foreground dark:bg-input/30 dark:border-input dark:hover:bg-input/50 h-9 px-4 py-1 has-[>svg]:px-3 cursor-pointer' 
                                                onClick={
                                                    () => {
                                                        setFormState((prev) => ({
                                                            ...prev,
                                                ...
                                                <Typography variant="c2" weight="semibold">{role.title}</Typography>
                                            </div>
                                            <div 
                                ...
                                if (index === 6) {
                                    ...
                                    <Button key="more-roles" variant="outline-black">
                                        <Typography variant="c2" weight="semibold">+{restRolesLength}</Typography>
                                    </Button>
                                    )
                                }
